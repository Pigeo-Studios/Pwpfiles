...
local belt_track_states = {
    belt_count = 0,
    belt_timer = -1
}

function belt_track_states.entry(this, context)
    context:runAnimation("static_belt_display", context:getTrack(STATIC_TRACK_LINE, BELT_TRACK), false, LOOP, 0)
    context:pauseAnimation(context:getTrack(STATIC_TRACK_LINE, BELT_TRACK))
end

function belt_track_states.update(this,context)
    context:resumeAnimation(context:getTrack(STATIC_TRACK_LINE, BELT_TRACK))
    context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, BELT_TRACK),0.1+0.5*belt_track_states.belt_count,false)
    context:pauseAnimation(context:getTrack(STATIC_TRACK_LINE, BELT_TRACK))
    if (belt_track_states.belt_timer ~= -1) then
        if (context:getCurrentTimestamp() - belt_track_states.belt_timer >= 2620) then
            belt_track_states.belt_count = 0
            belt_track_states.belt_timer = -1
        end
    end
end

function belt_track_states.transition(this,context,input)
    if (input == INPUT_RELOAD) then
        belt_track_states.belt_timer = context:getCurrentTimestamp()
    end
...
function gun_kick_state.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT) then
        ...
        if (belt_track_states.belt_count<16) then
            belt_track_states.belt_count = belt_track_states.belt_count + 1
        end
    end
    return nil
end
...
local M = {
    ...
    belt_track_states = belt_track_states,
...
function M:initialize(context)
    ...
    self.belt_track_states.belt_timer = -1
end
