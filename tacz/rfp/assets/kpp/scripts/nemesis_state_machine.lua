...
local BOLT_TRACK = increment(static_track_top)
...
local CHARGE_TRACK = increment(blending_track_top)
...
function bolt_caught_states.normal.update(this, context)
    ...
    local edge =50 --宽限，停止松手多久时触发，单位ms
    if (isNoAmmo(context)) then
    ...
    if ((this.bolt_caught_states.trigger)and(t2 - t1 > 2*itv+edge)) then
        local track = context:findIdleTrack(GUN_KICK_TRACK_LINE, false)
        context:runAnimation("shoot", track, true, PLAY_ONCE_STOP, 0)
        this.bolt_caught_states.trigger = false
        this.bolt_caught_states.proc = 0
    end

    local process_display = math.min(4 * this.bolt_caught_states.proc / max, 1)
    context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), process_display, true)
end
...
function bolt_caught_states.normal.entry(this, context)
    ...
    context:runAnimation("charge",context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), false, PLAY_ONCE_HOLD, 0)
...
function main_track_states.bolt.entry(this, context)
...
-- 在恰当的时候执行抛壳逻辑，然后退出状态
function main_track_states.bolt.update(this, context)
    local state = this.main_track_states.bolt
...
function main_track_states.bolt.transition(this, context, input)
...
function main_track_states.idle.update(this, context)
    if (context:isCharging()) then
        if (main_track_states.charge.can_charge) then
            context:trigger(this.INPUT_CHARING)
        end
    else
        main_track_states.charge.can_charge = true
    end
end
...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_BOLT) then
        context:runAnimation("bolt", context:getTrack(STATIC_TRACK_LINE, BOLT_TRACK), false, PLAY_ONCE_STOP, 0.2)
        -- 拉栓动画播放完后返回闲置态
    ...
    if (input == this.INPUT_CHARING) then
        main_track_states.charge.can_charge = false
        context:runAnimation("charge_in",context:getTrack(BLENDING_TRACK_LINE,CHARGE_TRACK),false,PLAY_ONCE_HOLD,0.3)
        return this.main_track_states.charge
    end
end

function main_track_states.charge.update(this, context)
...
-- 离开延迟扳机状态
function main_track_states.charge.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        context:stopAnimation(context:getTrack(BLENDING_TRACK_LINE,CHARGE_TRACK))
        context:runAnimation("charge_out", context:getTrack(BLENDING_TRACK_LINE,CHARGE_TRACK), false, PLAY_ONCE_STOP, 0.3)
        main_track_states.charge.can_charge = false
        return this.main_track_states.idle
    end
...
local M = {
    ...
    INPUT_CHARING = "input_charing",
    INPUT_CHARING_EXIT = "input_charing_exit"
}
